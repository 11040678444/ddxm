...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	/**
	 * Return worksheet info (Name, Last Column Letter, Last Column Customer, Total Rows, Total Columns)
	 *
	 * @param   string     $pFilename
	 * @throws   PHPExcel_Reader_Exception
	 */
...
//
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Customer'])) {
						$columnID = PHPExcel_Cell::stringFromColumnIndex($columnData_ss['Customer']-1);
					}
...
//						echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
			...
			if (isset($worksheet->Table->Row)) {
				...
				foreach($worksheet->Table->Row as $rowData) {
					...
					if (isset($row_ss['Customer'])) {
						$rowID = (integer) $row_ss['Customer'];
					}
//					echo '<b>Row '.$rowID.'</b><br />';
					...
					foreach($rowData->Cell as $cell) {
						...
						if (isset($cell_ss['Customer'])) {
							$columnID = PHPExcel_Cell::stringFromColumnIndex($cell_ss['Customer']-1);
						}
