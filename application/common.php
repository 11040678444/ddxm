...
function getOrderTracesByJson($order){
    $requestData= "{'OrderCode':'".$order["sn"]."','ShipperCode':'".$order["code"]."','LogisticCode':'".$order["express_code"]."'}";
 ...
 * @return url响应返回的html
 */
function sendPost1($url, $headers,$data=[]) {
    ...
    return empty(json_decode($data,true)) ? $data : json_decode($data,true);
}
